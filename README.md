...
### 数据可视化（@ohos/mpchart）
- **折线图**：电量趋势（纯线条 + 横线背景，无数据点）
- **柱状图**：每次会话的耗电/充电（使用红、充电绿、空闲黄）
- 支持拖拽、点击高亮等手势交互

### 数据统计
- 首页卡片：今日会话数 / 今日补能 / 今日耗能
- 历史页：总记录 / 总补能 / 总耗能（新能源汽车用 kWh）
...
- **导入**：从 JSON 恢复，设备 id 自动映射，保持设备-会话关联
...
| 文本解码 | `@ohos.util`（TextDecoder UTF-8） |
...
│   ├── BatteryInputDialog # 电量+续航输入弹窗
│   ├── DeviceSheet        # 设备创建/编辑表单（含新能源字段）
│   ├── SessionItem        # 历史会话条目（毫秒时间/电量/备注）
...
2. **记录会话**：设备卡片点「开始使用」或「开始充电」→ 输入当前电量（新能源汽车续航跟随）→ 进行中显示计时 → 点「结束会话」输入结束电量
3. **查看历史**：历史页查看折线趋势、柱状耗充、会话详情（毫秒时间/电量变化/备注）
