...
### 数据可视化（@ohos/mpchart）
- **折线图**：电量趋势（纯线条 + 横线背景，无数据点标记）
- **柱状图**：耗电图（使用红、闲置黄）与充电图（充电绿；背景仅横线）
- x 轴按序号均等间距排列，标签映射回真实时间（当天显示时:分，跨天显示月/日）；柱宽按槽位比例固定
- 点击数据点/柱子显示悬浮详情（数值、时间、备注），点击页面任意处或滚动即隐藏
- 支持左右拖拽浏览，禁用缩放与高亮

### 数据统计
- 首页卡片：今日会话数 / 今日补能 / 今日耗能
- 历史页：支持 **全部 / 本周 / 本月** 周期筛选，作用于统计卡片与会话列表（新能源汽车用 kWh）；图表数据范围独立——耗电图固定最近 7 个自然日，充电图取最近最多 20 条充电记录
